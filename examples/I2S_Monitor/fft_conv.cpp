...
{	int ich,ii,jj,kk;
    ...
    for(ich=0;ich<nch;ich++)
    {
      ...
      for(ii = 0; ii < mm; ii++) uu[ii]    = ov[ii + ich*mm];   // first overlap
      for(ii = 0; ii < ll; ii++) uu[mm+ii] = xx[ich + ii*nch];  // then input
      for(ii = 0; ii < mm; ii++) ov[ii + ich*mm] = xx[ich + (ll+ii)*nch];  //store last part from input to overlap
      //
      float *zp = &zz[ich*(nf*nn) + ij*nn];  // pointer where actual spectrum will go
